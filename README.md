...
> **Status:** early. Ships with a profile for Le Mans Ultimate and session
> plugins for iRacing, Assetto Corsa, Automobilista 2 and Project CARS. Other
> sims work by adding a profile, which takes about a minute — see
> [Adding your sim](#adding-your-sim).

### What's coming

- **More sim profiles and session plugins**, so more games work without you
  adding anything. A profile is what lets PitRadio type into a game's chat box;
  a plugin is what lets the engineer know who is on track around you. The
  quickest way to get your sim on that list is
  [an issue](#reporting-a-fault) saying what it is called.
- **AI Coaching** — a paid service that drives your car in an offline practice
  session, sets a quick lap on your own wheel, and then coaches you until you
  can match it. What the coach does today is compare you against a lap somebody
  already drove; this sets the reference itself, on your car and your setup.
- **AI Engineer** — a paid service that reads the telemetry as the race
  happens and answers questions about it. Not a fixed list of things it can be
  asked, like today's engineer: strategy, what the weather is doing to your
  tyres, whether that stop works, asked in your own words and answered from
  what is actually going on.

Both need a model large enough that it cannot run on the machine also running
the sim, so both are GPU-backed and metered — you pay for the time you use,
and nothing else about PitRadio changes. Everything described below runs on
your own machine and always will.
...
start by saying something. Two recorded voices ship with it — Norman and
Claudia — and it will use a Windows voice for anything a recording cannot
cover, such as a driver's name.
...
How it sounds is set in **Settings → Voice**, not on the Engineer tab: the
engineer and the coach speak in the same voice, out of the same queue, so it is
one setting for the app rather than one per feature.

**[docs/engineer.md](docs/engineer.md)** is the full guide: talking to it,
writing your own routine phrases, other languages, and installing a recorded
voice pack.
...
> *You gave up mid-corner speed because you were still on the brake at the
> apex, and they apex later so their exit straightens out sooner.*
...
**The circuit is worked out from your own laps.** After a few laps PitRadio
knows the shape of the road and cuts it into segments — corners and the
straights between them — so it works on any track in any sim, including ones
nobody has ever catalogued. Where there is a catalogue you get names instead of
numbers.
...
Hold the same key and the other PitRadio users in your session hear you. It is
...
alongside. See **[docs/voice.md](docs/voice.md)**.
...
PitRadio is made by **AxisTaylor, LLC** and is proprietary software. Installing or
